...
class Region:
    def __init__ (self, uSize, uInputsNumber, *args, **kwargs):
        ...
        self._permanence_inc = 0.1
        self._permanence_dec = 0.1
    ...
    def max_duty_cycle (self, uColumns):
        return max ([d._active_duty_cycle for d in uColumns])

    def update_active_duty_cycle (self, uColumn):
    ...
    def learn (self):
        for column in self._active_columns:
            for s in column._potential_synapses:
                if s._input == 1:
                    s._permanence = s._permanence + self._permanence_inc
                    if s._permanence > 1: s._permanence = 1.0
                    
                else:
                    s._permanence = s._permanence - self._permanence_dec
                    if s._permanence < 0: s._permanence = 0.0

        for column in self._columns_collection:
            column._min_duty_cycle = 0.01 * self.max_duty_cycle (self.neighbours (column))
            column._active_duty_cycle = column.up
